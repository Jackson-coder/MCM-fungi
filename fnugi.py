...
def compare(decomposition, litter, record_x, record_y):
...
temperature_high = temperature_low + \
    np.random.normal(loc=20, scale=10, size=50)
...
width_high = np.random.normal(loc=10, scale=4, size=50) / 10 + width_low
...
for i in range(50):
    fnu = Q1.fnugis(extension_rate[i], temperature_high[i],
                    ...
                    width_high[i], width_low[i], width_now, 0.00055, 0.0055,
                    number_now[i], K, moisture_tolerance[i], decomposition_rate[i])
    F.append(fnu)

file_csv = ['马瑙斯.csv', '洛杉矶.csv', '福克斯.csv', '开罗.csv', '西雅图.csv']
...
for record in litter_record:
    compare(record[0], record[1], record[2], record[3])
